...
def _extract_train_snapshots(feed_id: str, feed: NYCTFeed) -> list[dict]:
    snapshots: list[dict] = []
        ...
        snapshots.append(
            {
                ...
                "last_position_update": (
                    _iso_or_none(trip.last_position_update) if trip.underway else None
                ),
                "next_stop_arrival_time": (
                    _iso_or_none(stop_updates[0].arrival) if stop_updates else None
                ),
                "next_stop_departure_time": (
                    _iso_or_none(stop_updates[0].departure) if stop_updates else None
                ),
                "shape_id": trip.shape_id,
                "direction": trip.direction,
                "current_stop_sequence": (
                    trip.current_stop_sequence_index if trip.underway else None
                ),
...
def _poll_feed_sync(feed_id: str, url: str) -> tuple[str, list[dict], datetime | None, str | None]:
    try:
        ...
        snapshots = _extract_train_snapshots(feed_id, feed)
        return status, snapshots, feed_timestamp, error
...
async def _persist_feed_poll(
    ...
    if snapshots and feed_timestamp and status == "healthy":
        await upsert_train_states(feed_id, feed_timestamp, snapshots)
        logger.info("Upserted %d trains from feed %s", len(snapshots), feed_id)
        return snapshots
...
async def poll_all_feeds() -> None:
    ...
    snapshots_for_enrichment: list[dict] = []
    for (feed_id, _), (status, snapshots, feed_timestamp, error) in zip(feed_items, results):
        persisted = await _persist_feed_poll(feed_id, status, snapshots, feed_timestamp, error)
        snapshots_for_enrichment.extend(persisted)

    if snapshots_for_enrichment:
        await finalize_poll_enrichment(snapshots_for_enrichment)
...
async def collector_loop(stop_event: asyncio.Event) -> None:
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            deleted = await prune_stale_train_states()
            if deleted:
                logger.info("Pruned %d stale trains", deleted)
